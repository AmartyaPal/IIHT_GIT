...
namespace ProjectManager.BC
{
    public class ProjectBC
    {
        ...
        public int UpdateProjectDetails(MODEL.Project project)
        {
            using (dbContext)
            {
                var editProjDetails = (from record in dbContext.Projects
                                       where record.Project_ID.ToString().Contains(project.ProjectId.ToString())
                                       select record).First();
        ...
        public int DeleteProjectDetails(MODEL.Project project)
        {
            using (dbContext)
            {
                ...
                var editDetails = (from record in dbContext.Projects
                                   where record.Project_ID == project.ProjectId
                                   select record).First();
                if (editDetails != null)
                {
