...
namespace ProjectManager.BC
{
    public class TaskBC
    {
            ...
            using (dbContext)
            {
                ...
                else
                {
                    ...
                    var editDetails = (from record in dbContext.Users
                                       where record.User_ID.ToString().Contains(task.User.UserId.ToString())
        ...
        public int UpdateTaskDetails(Task task)
        {
            using (dbContext)
            {
                var editDetails = (from record in dbContext.Tasks
                                   where record.Task_ID.ToString().Contains(task.TaskId.ToString())
                                   select record).First();
                if (editDetails != null)
                {
                ...
                var editDetailsUser = (from record in dbContext.Users
                                       where record.User_ID.ToString().Contains(task.User.UserId.ToString())
                                       select record).First();
        ...
        public int DeleteTaskDetails(Task task)
        {
            using (dbContext)
            {
                var deleteTask = (from record in dbContext.Tasks
                                  where record.Task_ID.ToString().Contains(task.TaskId.ToString())
                                  select record).First();
                if (deleteTask != null)
                {
